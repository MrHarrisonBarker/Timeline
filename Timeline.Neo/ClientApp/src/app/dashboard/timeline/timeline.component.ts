...
export class TimelineComponent implements OnInit
{
  ...
  {
    this.flaggedJobs = this.auth.User.assignments.filter(x => x.flagged);
  }
  ...
  private filterOverDeadline ()
  {
    let now = new Date();
    console.log(now);
    this.overDeadlineJobs = this.auth.User.assignments.filter(x => new Date(x.deadline) < now && new Date(x.deadline) > new Date(null));
  }
